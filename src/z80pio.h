...
void _z80pio_write_ctrl(z80pio_t* pio, int port_id, uint8_t data) {
    ...
    if (p->expect_io_select) {
        ...
        //std::cout << "Set IO mask " << mask << " (1 = input, 0= output)" << std::endl;
    }
...
uint8_t _z80pio_read_data(z80pio_t* pio, int port_id) {
    ...
    switch (p->mode) {
        ...
        case Z80PIO_MODE_BITCONTROL:
            {
            uint8_t output = (p->input & p->io_select) | (p->output & ~p->io_select);
            // std::bitset<8> portbits(output);
            // std::cout << "Read port data " << portbits << std::endl;
            return output;
            }
        default:
            _Z80PIO_UNREACHABLE;
...
uint64_t _z80pio_set_port_output_pins(z80pio_t* pio, uint64_t pins) {
    for (int port_id = 0; port_id < Z80PIO_NUM_PORTS; port_id++) {
        ...
        //std::bitset<8> portbits(data);
        if (port_id == 0) {
            //std::cout << "Setting port A to " << portbits << std::endl;
            Z80PIO_SET_PA(pins, data);
        }
        else {
            //std::cout << "Setting port B to " << portbits << std::endl;
